...
function goBackToEntityPage(entityId) {
...
function checkSessionTimer(username, loginTime) {
    var timer;
    window.clearInterval(timer);
    timer = window.setInterval(function() {
            checkSession(username, loginTime)
    }, 10000);  
}

function checkSession(username, loginTime) {
        $.post("ajax/checkSession.php", { username: username, loginTime:loginTime}, function(data) {
            if(data !== null && data !== "") {
                //alert(data);
                if (data == "DOES NOT EXIST") {
                    window.location.href = "logout.php?reason=sessnexp";
            }
                }
            });
};